        ...
        while True:
            ...
            while True:
                try:
                    ...
                    print('client %s msg:%s' % (self.client_address, head_struct))
    ...
    def login(self,args):
        ...
        if tag:
            ...
            for item in user_info_li:
                ...
                # self.request.send(bytes("用户名或密码错误!",encoding='utf-8'))
                self.request.send(bytes("False",encoding='utf-8'))
        else:
            print("\033[1;31m用户不存在!\033[0m")
            # self.request.send(bytes("用户不存在!",encoding='utf-8'))
            self.request.send(bytes("False",encoding='utf-8'))
...
def main():
    ...
    入口函数
    """
    s = socketserver.ThreadingTCPServer(('127.0.0.1',8080),MYTCPServer)
    s.serve_forever()
